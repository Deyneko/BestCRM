//программа для поиска сайтов с помощью матриц, типа match
//оптимизирована для записи результата в бинарный файл
//Здесь матрицы разбиты на тройки, сиквенс - + два элемента вперед.
// выводит сиквенсы как матч
//здесь уже берет несколько сиквенсов
//v1.5 - Формируем стрроку вручную и пишем
//v1.6 - проверка достаточен ли кор
//13.06 - Неправильно выводит начало сайта на обр посл в бинарном файле - Fixed
//20.11 - Добавил статистику по каждому типу сайтов
//5.12 -  Дополнил структуру матрицы полем AC и OrigLength - первоначальная длина
//29.04 - Добавил формат выдачи для создания табицы вес-частота для mcatch (выдает матрица, вес), позиции не надо
//v1.7 Apr 29 2004 - Теперь весь ход событий будет определяться define ...
//12.05 [+] - теперь надо будет указывать "+" чтобы выводить все сайты, по умочанию нет
//v2.0 May 14 - обрабатывает много сиквенсов, пишет в любые комбинации файлов.
//v2.1 July 2 - добавил статистику по обработанному файлу с последовательностями и Verbose
//v2.2 July 17 - добавил выдачу нуклеотидного состава, суммарно для всех последов при Verbose >= 2;
//July 29 2008 - Объеденил с вариантом для Линуха, теперь надо просто закоментить ненужное
//v2.3 22.Oct.2008 - Выдача более компактная для бинари - 10 байт ( - Есть идея сдлать мега компактную бинарную выдачу. тип матрицы, позиция (локально сортировать?), вес - относ предыдущего сайта.
//                 - Сейчас пока определяется define, так что вопрос надо ли добавить ключ /BinOnly и т.д. - выдача только бинарного файла, ...
//                 - Добавил вывод через буфер, стало реально быстрее, круто стало!!!
//v2.4 15.Oct.2008 - Теперь путь к mm.dat будет относительно .lib файла (т.е. будет там же где и либ, а не в текущей дир).
//    -  пишем обратную последовательность(и) при  #define EncodeSeq, т.е. просто делает комплиментраную последовательность.
//23 Oct 09 -  Добавил подсчет динуклеотидного состава суммарно для всех последов, при Verbose >= 2
//v2.41 2 Dec 09 - Добавил расслабление порогов относительно профиля /mdprf. /prf Val - применить это значение для всех матриц из .либ (вместо профиля) - это чтобы профиль не менять при разных пробах
//v2.42 23 Mar 10 - Добавил выдачу в текстовый файл "от до"
//v2.5 13 Jul 10 - Добавил стандартный путь до матриц mm.dat, чтобы копии не держать везде. Имя для .dat генерируется автоматически в зависимости от .lib.
//v2.6 3 Feb 11 - Добавил процедуру подсчета GC состава в целом и окне по ключу
//    19 Okt 11 - добавил подсчет триплетов.
//
// !!  сделать принт всех параметров запуска, вкл все ключи !!

//!! Автоматиески определять скор для кора, и из профиля его больше не брать

//!!Выводить параметры изменения профиля если заданы.
//!! закачивать всегда полный лист матриц, делать два списка полный и по профилю

//Добавить Имя сиквенса в бинарный файл. Писать MaxLineLength байт перед сайтами. Оформить в виде функции
//Фильтровать сайты от пересечений (по ключу)
//Автоматически вычислять скор для кора от полного скрора Кор=Полн-макс(края)

// стандартный путь до случ послед?
//!!! Сделать ключ, чтобы искала число сайтов/частоту на сиквенс, порог тогда не надо. - наверно лучше в mmtools.
//!!! - ввести обратное округление для порогов: 0.90 = 0.8950000

//!!! - если сиквенс короткий до добавим в конец N до норм длины или перейти к след. а не выход
//!!! - в чтении EMBL может быть многострочное описание под знаком SQ/SQ/...
//!!! - подумать как включить 16-буквенные последовательности.
//    -  опция подобрать пороги. напр 1 на тыс п.н. и записать их в профиль! - Сделал в MMtools
//!!! - многопоточность? - когда перейду на другой компилятор. Потоки берут последовательно коровые пятерки и ищут их

//!!!!!! - В чтении матриц добавить попроще формат, просто 4 числа через пробелы (.lib - чтоб в ручную матрици делать)
