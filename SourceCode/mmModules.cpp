...
//Доработана для Продорик (MaxMatrLength=35->45), tags NewModule EndNewModule
// про укскорение:
//   - к сайт елементу добавить поле где след сайт такого же типа.
//
//
...
float PwmThrRlx=0.6, LenThrRlx=3, CSThrRlx=0.8; // Maximum relaxation values. PwmThrMin=1-PwmThrRlx
...
printf ("MinCoverY %3.2f, MaxCoverN %3.2f, FreqDiff %3.2f WindowLength %d",MinCoverY,MaxCoverN,FreqDiff,WindowLength);
if (FullOutput) printf ("FullOutput = %d",FullOutput);
printf ("\n");
...
} // потом переставить порядок суммирования - непомогло, та же скорость :(
...
// print distribution function, this is for paper only
printf ("Distribution function, this is for paper only\n");
for (i1=0;i1<=PwmThrNum-1;i1++) for (i2=0;i2<=PwmThrNum-1;i2++)
{m=0; for (SN=0;SN<=InfoY->NumofSeq-1;SN++) //по всем сиквенсам
      if (CEDistrY[SN]->CETable[i1][i2][0][0]>=MinSitesY) m++;
n=0;  for (SN=0;SN<=InfoN->NumofSeq-1;SN++) //по всем сиквенсам
      if (CEDistrN[SN]->CETable[i1][i2][0][0]>=MaxSitesN) n++;
//printf ("%3d %3d %10f %10f\n",i1,i2,m/(float)InfoY->NumofSeq,m/(float)InfoN->NumofSeq);
printf ("%10f %10f %10f %10f\n",1-i1*PwmThrRlx/(float)PwmThrNum,1-i2*PwmThrRlx/(float)PwmThrNum,m/(float)InfoY->NumofSeq,n/(float)InfoN->NumofSeq);
}
//exit (0);
